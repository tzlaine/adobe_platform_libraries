...
LRESULT CALLBACK popup_subclass_proc(HWND window, UINT message, WPARAM wParam, LPARAM lParam, UINT_PTR ptr, DWORD_PTR)
{
...
void initialize(adobe::popup_t& control, adobe::platform_display_type parent)
{
    assert(!control.control_m);
...
void message_menu_item_set(adobe::popup_t& p)
{
    ...
    for (auto first=p.menu_items_m.begin(), last=p.menu_items_m.end(); first != last; ++first)
    {
        /* TODO
    ...
    // TODO ::SendMessage(p.control_m, CB_SETCURSEL, 0, 0);
    ...
    for (; first != last; ++first)
    {
        if ((*first).second == value)
        {
            if (custom_m)
            {
                ...
                // TODO ::SendMessage(control_m, CB_DELETESTRING, 0, 0);
            }
            ...
            // TODO ::SendMessage(control_m, CB_SETCURSEL, index, 0);
...
void popup_t::display_custom()
{
    ...
    // TODO ::SendMessageA(control_m, CB_INSERTSTRING, 0, (LPARAM)custom_item_name_m.c_str());

    type_2_debounce_m = true;

    // TODO ::SendMessage(control_m, CB_SETCURSEL, 0, 0);
...
void popup_t::select_with_text(const std::string& text)
{
    ...
    long old_index = 0;// TODO (static_cast<long>(::SendMessage(control_m, CB_GETCURSEL, 0, 0)));

    // TODO ::SendMessageW(control_m, CB_SELECTSTRING, (WPARAM) -1, (LPARAM) hackery::convert_utf(text.c_str()).c_str());

    if (value_proc_m.empty())
        return;

    long new_index = 0;// TODO (static_cast<long>(::SendMessage(control_m, CB_GETCURSEL, 0, 0)));
