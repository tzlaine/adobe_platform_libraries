...
/****************************************************************************************************/
extents_t calculate_edit_bounds(platform_display_type control, int cols, int rows);
...
void edit_text_t::measure(extents_t& result)
{
    ...
    measure_label_text(get_label(), label_bounds);
    //
...
void edit_text_t::set_field_text(const std::string& text)
{
    ...
    // TODO ::SendMessageW(control_m, WM_SETTEXT, 0, (LPARAM) hackery::convert_utf(text.c_str()).c_str());
