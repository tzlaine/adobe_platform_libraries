...
{
    if (x.type_info() == boost::typeindex::type_id<double>())
        return number_format<double>(format_m, x.cast<double>());
    else
...
{
    if (the_type.type_info() == boost::typeindex::type_id<double>())
        return any_regular_t(number_parse<double>(format_m, str));
    else
