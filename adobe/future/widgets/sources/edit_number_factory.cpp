...
{
    adobe::sheet_t&         layout_sheet(token.client_holder_m.layout_sheet_m);
    ...
    // generated.
    {
        ...
        if (layout_sheet.count_interface(cell) == 0) {
            ...
            /*
                REVISIT (sparent) : We're going to attach to the cell and we need the value type
                to be correct. So we force an update. This isn't very efficient!
            */
            layout_sheet.update();
        }
